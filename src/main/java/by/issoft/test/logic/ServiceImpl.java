...
import by.issoft.test.dao.CSVDAOImpl;
import by.issoft.test.dao.DAO;
import by.issoft.test.dao.DAOHolder;
...
public class ServiceImpl implements Service {
    ...
    @Override
    public BlockingQueue<DayTotalProductPrice> getDayTotalProductPrices(String ordersPath, String orderItemsPath, String productsPath) {
        BlockingQueue<DayTotalProductPrice> dayTotalProductPrices = new LinkedBlockingQueue<>();
        DAO dao = DAOHolder.getInstance().getDAO();

        List<Order> orders = dao.getOrders(ordersPath);
        List<OrderItem> orderItems = dao.getOrderItems(orderItemsPath);
        List<Product> products = dao.getProducts(productsPath);

        if (orders == null || orderItems == null || products == null)
            return dayTotalProductPrices;

        dayTotalProductPrices = getDayTotalProductPrices(getItemsWPricePerDay(orders, orderItems, products));
